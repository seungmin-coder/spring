...
public class ReviewReplyController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command.equals("ReviewReplyinsert")) {
			...
			ReviewReplyDto dto = new ReviewReplyDto(rv_no,mb_id,RR_content);
			int res = biz.insert(dto);
			System.out.println("컨트롤러 res 부분 "+res);
			
			 if(res>0) { 
				 jsResponse(response, "ReviewController.do?command=review_list", "글작성 성공");
			 }else {
				 jsResponse(response, "ReviewController.do?command=review_list","글작성 실패"); 
				 }
			...
			int res = biz.delete(RR_no);
			if(res>0) {
				jsResponse(response, "ReviewController.do?command=review_list", "삭제되었습니다.");
			}
		...
		if(res>0) {
			jsResponse(response, "ReviewController.do?command=review_list", "수정성공");
		}else {
			jsResponse(response, "ReviewController.do?command=review_list", "수정실패");
		}
			...
			if(res>0) {
				jsResponse(response, "ReviewController.do?command=review_list", "답글성공");
			}else {
				jsResponse(response, "ReviewController.do?command=review_list", "답글실패");
			}
		}
